...
export default function Sidebar({ activeTab, setActiveTab, productsCount, categoriesCount }) {
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-mehmon-sidebar/95 backdrop-blur-lg border-t border-mehmon-border px-4 py-2 flex items-center justify-around shadow-2xl transition-colors duration-300">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
